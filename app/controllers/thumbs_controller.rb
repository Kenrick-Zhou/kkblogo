class ThumbsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @thumb.save
        ...
        format.json { render :show, status: :created, location: @thumb }
      else
  ...
  def update
    respond_to do |format|
      if @thumb.update(thumb_params)
        ...
        format.json { render :show, status: :ok, location: @thumb }
      else
  ...
  def destroy
    @thumb.destroy
    respond_to do |format|
      format.html { redirect_to thumbs_url, notice: 'Thumb was successfully destroyed.' }
  ...
  private
    ...
    def thumb_params
      params.require(:thumb).permit(:name, :ext, :width, :height)
    end
end
